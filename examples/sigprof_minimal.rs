// Minimal SIGPROF reproducer: no pprof-rs, no handler cycling, no threads
// doing profiler work. If this still crashes on macos-15-arm64 (Rosetta 2)
// under x86_64-apple-darwin with the same rip-in-stack / BUS_ADRERR pattern,
// it proves the SIGBUS is a Rosetta signal-delivery bug that hits *any*
// SIGPROF delivery, not something specific to pprof-rs.
//
// Usage:
//   cargo run --example sigprof_minimal --target x86_64-apple-darwin
...
fn main() {
    ...
    // Background CPU burners so SIGPROF fires often.
    ...
    // Burn CPU on the main thread and print SIGPROF count periodically.
    ...
    write_all(2, b"sigprof_minimal: completed without a crash, total sigprofs=");
    write_dec(2, SIGPROF_COUNT.load(Ordering::Relaxed));
    write_all(2, b"\n");
}
