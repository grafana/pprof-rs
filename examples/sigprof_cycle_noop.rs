// Second isolation step: cycle sigaction(SIGPROF) + setitimer(ITIMER_PROF)
// at the same cadence as pprof::ProfilerGuard::new(999) / drop, but with a
// no-op SIGPROF handler. No pprof-rs.
//
// - sigprof_minimal (install once, no cycling)   -> no crash
// - sigprof_cycle_noop (cycle, no pprof work)    -> this file
// - sigbus_repro (cycle, full pprof-rs handler)  -> crashes
//
// If this one crashes too, the culprit is the cycling mechanic (sigaction
// storm). If it does not, the culprit is inside pprof-rs's handler body
// (RwLock + backtrace::trace_unsynchronized + sample()).
...
fn main() {
    ...
    // Same cadence as tests/sigprof_race.rs: 8000 cycles of
    //   install handler -> start timer -> 50k busy loop -> stop timer -> SIG_IGN
    for i in 0..8000 {
        install_sigprof();
        start_timer(999);
        for _ in 0..50_000 {
    ...
    write_all(2, b"sigprof_cycle_noop: completed without a crash, sigprofs=");
    write_dec(2, SIGPROF_COUNT.load(Ordering::Relaxed));
    write_all(2, b"\n");
}
